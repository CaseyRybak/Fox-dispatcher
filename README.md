...
- поиск по имени или `fox_id` и фильтры по локации, цвету и добыче существуют только на Сводке и меняют только видимый список рейтинга; лидер, показатели и расчёты всегда строятся по полному журналу, а раздел `Параметры` показывает все записи и не отображает состояние фильтров;
- строка рейтинга показывает название локации, если все наблюдения лисы сделаны в одном месте, и `Несколько локаций`, если мест несколько; этот контекст строится по полному журналу и не меняется от фильтров;
