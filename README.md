...
4. Откройте `Наблюдения`, измените оценку `obs_005` с 3 на 10 и вернитесь в сводку: лидером станет Лиса 4 (`fox_004`) с индексом `8,0`.
...
7. На вкладке `Наблюдения` откройте импорт, вставьте JSON, проверьте предпросмотр и только затем подтвердите замену; `Экспортировать все наблюдения` создаёт `fox-dispatcher-observations.json`.
