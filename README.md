...
- поиск по имени или `fox_id` и фильтры по локации, цвету и добыче используют одну область расчёта;
- у каждой лисы доступны исходные записи и прозрачная арифметика результата; периодические значения показываются до десятых с явной отметкой округления;
