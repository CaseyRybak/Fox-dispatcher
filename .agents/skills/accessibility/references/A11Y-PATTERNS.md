...
Offer a single-pointer alternative for every action triggered by dragging (WCAG 2.5.7).
...
Apply the same pattern to sliders, map panning, colour pickers, and similar drag-based widgets by providing an equivalent click, tap, or keyboard path.
