async (page) => {
  ...
  assert(
    ...
    `The deployed product requested external origins: ${unexpectedOrigins.join(", ")}.`,
  );

  if (expectedOrigin.startsWith("https://")) {
    const headers = await documentResponse?.allHeaders();
    assert(
      headers?.["content-security-policy"]?.includes("connect-src 'none'") &&
        headers["content-security-policy"].includes("frame-ancestors 'none'"),
      "The deployed document is missing the required Content-Security-Policy.",
    );
    assert(
