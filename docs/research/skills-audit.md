...
Fox Dispatcher — интерактивное веб-приложение «Лисий диспетчер»: редактируемые наблюдения, объяснимый расчёт подозрительности, сводка по локациям и встроенный AI Worklog. Этот документ фиксирует выбор скиллов; он не является планом реализации приложения.
...
- `finishing-a-development-branch`: понадобится только на этапе релиза.
